function generateMJML(blocks, globalSettings = {}) {
  const {
    width = 600,
  ...
  const isGoogleFont = !['Arial', 'Georgia', 'Tahoma', 'Times New Roman', 'Helvetica Neue', 'Trebuchet MS'].includes(fontFamily);
  const fontStack = `${fontFamily}, Arial, sans-serif`;
  const sections = blocks.map(b => blockToMJML(b, { fontStack, emailAreaColor, linkColor })).filter(Boolean).join('\n');
  ...
  <mj-head>
    ...
    ${isGoogleFont ? `<mj-font name="${fontFamily}" href="https://fonts.googleapis.com/css2?family=${encodeURIComponent(fontFamily)}:wght@300;400;500;600;700&display=swap" />` : ''}
...
function blockToMJML(block, ctx) {
  ...
  switch (block.type) {
    case 'logo': {
      const src = p.src && p.src !== 'placeholder' ? p.src : 'https://placehold.co/180x60/e8e8e8/aaaaaa?text=Logo';
    ...
    case 'title': {
      const ff = p.fontFamily === 'global' ? ctx.fontStack : `${p.fontFamily}, Arial, sans-serif`;
      return section(bg, pad(p), `<mj-text align="${p.align}" color="${p.color}" font-size="${p.fontSize}" font-weight="${p.fontWeight || '700'}" font-family="${ff}" line-height="1.25" padding="0">${p.content}</mj-text>`);
    }
    case 'text': {
      const ff = p.fontFamily === 'global' ? ctx.fontStack : `${p.fontFamily}, Arial, sans-serif`;
      return section(bg, pad(p), `<mj-text align="${p.align}" color="${p.color}" font-size="${p.fontSize}" line-height="${p.lineHeight || '1.6'}" font-family="${ff}" padding="0">${p.content}</mj-text>`);
    }
    case 'image': {
      const src = p.src && p.src !== 'placeholder' ? p.src : 'https://placehold.co/600x300/e8e8e8/aaaaaa?text=Image';
      const href = p.link ? ` href="${p.link}"` : '';
    ...
    case 'video': {
      const thumb = p.thumbnailSrc && p.thumbnailSrc !== 'placeholder' ? p.thumbnailSrc : 'https://placehold.co/600x338/111827/ffffff?text=Watch+Video';
